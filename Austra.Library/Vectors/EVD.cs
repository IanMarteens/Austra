        ...
        double* a, double* d, double* e, int r)
    {
        ...
        for (int i = r - 1; i > 0; i--)
        {
            ...
            else
            {
                ...
                {
                    V8d sum = V8d.Zero, scl = V8.Create(1.0 / scale);
                    for (int t = i & Simd.MASK8; m < t; m += V8d.Count)
                    {
                        V8d dvec = Avx512F.LoadVector512(d + m) * scl;
                        sum = Avx512F.FusedMultiplyAdd(dvec, dvec, sum);
                        Avx512F.Store(d + m, dvec);
                    }
                    h = V8.Sum(sum);
                }
                else if (Avx.IsSupported)
                {
                    V4d sum = V4d.Zero, scl = V4.Create(1.0 / scale);
                ...
                for (int j = 0; j < i; j++, aj += r)
                {
                    ...
                    if (Avx512F.IsSupported)
                    {
                        V8d vg = V8d.Zero, vf = V8.Create(f);
                        for (int t = ((i - j - 1) & Simd.MASK8) + j + 1; k < t; k += V8d.Count)
                        {
                            V8d va = Avx512F.LoadVector512(aj + k);
                            vg = Avx512F.FusedMultiplyAdd(Avx512F.LoadVector512(d + k), va, vg);
                            Avx512F.Store(e + k, Avx512F.FusedMultiplyAdd(va, vf, Avx512F.LoadVector512(e + k)));
                        }
                        g += V8.Sum(vg);
                    }
                    else if (Avx.IsSupported)
                    {
                        V4d vg = V4d.Zero, vf = V4.Create(f);
                ...
                if (Avx512F.IsSupported)
                {
                    V8d s = V8d.Zero, scl = V8.Create(1.0 / h);
                    for (int t = i & Simd.MASK8; m < t; m += V8d.Count)
                    {
                ...
                {
                    V4d s = V4d.Zero, scl = V4.Create(1.0 / h);
                    for (int t = i & Simd.MASK4; m < t; m += V4d.Count)
                    {
        ...
        double* a, double* d, double* e, int r)
    {
            ...
            if (m > l)
            {
                ...
                do
                {
                    ...
                    for (i = m - 1; i >= l; i--, ai -= r)
                    {
                        ...
                        if (Avx512F.IsSupported)
                        {
                            V8d vs = V8.Create(s), vc = V8.Create(c);
                            for (int t = r & Simd.MASK8; k < t; k += V8d.Count)
                            {
                        ...
                        {
                            V4d vs = V4.Create(s), vc = V4.Create(c);
                            for (int t = r & Simd.MASK4; k < t; k += V4d.Count)
                            {
        ...
        for (int i = 0; i < r - 1; i++)
        {
            ...
            if (k != i)
            {
                ...
                int j = 0;
                if (Avx512F.IsSupported)
                    for (int t = r & Simd.MASK8; j < t; j += V8d.Count)
                    {
                        V8d v = Avx512F.LoadVector512(ai + j);
                        Avx512F.Store(ai + j, Avx512F.LoadVector512(ak + j));
                        Avx512F.Store(ak + j, v);
                    }
                else if (Avx.IsSupported)
                    for (int t = r & Simd.MASK4; j < t; j += V4d.Count)
                    {
    ...
    private unsafe static void ReduceToHessenberg(double* a, double* h, double* ort, int rank)
    {
        for (int m = 1, high = rank - 1; m < high; m++)
        {
            ...
            if (scale != 0.0)
            {
                ...
                if (Avx512F.IsSupported)
                {
                    V8d vsc = V8.Create(1d / scale), vhh = V8d.Zero;
                    for (int t = ((rank - m) & Simd.MASK8) + m; i < t; i += V8d.Count)
                    {
                        V8d v = Avx512F.LoadVector512(h + mm1O + i) * vsc;
                        Avx512F.Store(ort + i, v);
                        vhh = Avx512F.FusedMultiplyAdd(v, v, vhh);
                    }
                    hh = V8.Sum(vhh);
                }
                else if (Avx.IsSupported)
                {
                    V4d vsc = V4.Create(1d / scale), vhh = V4d.Zero;
                ...
                for (int j = m, jO = m * rank; j < rank; j++, jO += rank)
                {
                    ...
                    if (Avx512F.IsSupported)
                    {
                        V8d vf = V8.Create(f);
                        for (int t = ((rank - m) & Simd.MASK8) + m; i < t; i += V8d.Count)
                            Avx512F.Store(h + jO + i, Avx512F.FusedMultiplyAddNegated(
                                Avx512F.LoadVector512(ort + i), vf,
                                Avx512F.LoadVector512(h + jO + i)));
                    }
                    else if (Avx.IsSupported)
                    {
                        V4d vf = V4.Create(f);
        ...
        for (int m = rank - 2, mm1O = (m - 1) * rank; m >= 1; m--, mm1O -= rank)
        {
            if (h[mm1O + m] != 0.0)
            {
                CopyBlock(ort + m + 1, h + mm1O + m + 1,
                    (uint)(rank - m) * sizeof(double));
                for (int j = m, jO = m * rank; j < rank; j++, jO += rank)
                {
                    ...
                    new Span<double>(ort + m, rank - m)
                        .MulAddStore(g, new Span<double>(a + jO + m, rank - m));
                }
            }
        }
    }
            ...
            else
            {
                ...
                for (int k = m; k < n; k++)
                {
                    ...
                    if (s != 0.0)
                    {
                        ...
                        int upper = Min(n, k + 3);
                        int i = 0;
                        if (Avx512F.IsSupported && upper >= V8d.Count)
                        {
                            V8d vx = V8.Create(x), vy = V8.Create(y), vz = V8.Create(z);
                            V8d vr = V8.Create(r), vq = V8.Create(q);
                            for (int top = (upper + 1) & Simd.MASK8; i < top; i += V8d.Count)
                            {
                                V8d v1 = Avx512F.LoadVector512(h + kO + i);
                                V8d v2 = Avx512F.LoadVector512(h + kp1O + i);
                                V8d vp = V8.FusedMultiplyAdd(vy, v2, vx * v1);
                                if (notlast)
                                {
                                    V8d v3 = Avx512F.LoadVector512(h + kp2O + i);
                                    vp = V8.FusedMultiplyAdd(vz, v3, vp);
                                    Avx512F.Store(h + kp2O + i, Avx512F.FusedMultiplyAddNegated(vp, vr, v3));
                                }
                                Avx512F.Store(h + kO + i, v1 - vp);
                                Avx512F.Store(h + kp1O + i, Avx512F.FusedMultiplyAddNegated(vp, vq, v2));
                            }
                        }
                        else if (Avx.IsSupported && upper >= V4d.Count)
                        {
                            V4d vx = V4.Create(x), vy = V4.Create(y), vz = V4.Create(z);
                            V4d vr = V4.Create(r), vq = V4.Create(q);
                            for (int top = (upper + 1) & Simd.MASK4; i < top; i += V4d.Count)
                            {
                        ...
                        if (Avx512F.IsSupported)
                        {
                            ...
                            for (int top = rank & Simd.MASK8; i < top; i += V8d.Count)
                            {
                                V8d v1 = Avx512F.LoadVector512(a + kO + i);
                                V8d v2 = Avx512F.LoadVector512(a + kp1O + i);
                                V8d vp = Avx512F.FusedMultiplyAdd(vx, v1, vy * v2);
                                if (notlast)
                                {
                                    V8d v3 = Avx512F.LoadVector512(a + kp2O + i);
                                    vp = Avx512F.FusedMultiplyAdd(vz, v3, vp);
                                    Avx512F.Store(a + kp2O + i,
                                        Avx512F.FusedMultiplyAddNegated(vp, vr, v3));
                                }
                                Avx512F.Store(a + kO + i, v1 - vp);
                                Avx512F.Store(a + kp1O + i,
                                    Avx512F.FusedMultiplyAddNegated(vp, vq, v2));
                            }
                        }
                        else if (Avx.IsSupported)
                        {
